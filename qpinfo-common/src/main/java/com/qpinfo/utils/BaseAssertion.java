...
import com.qpinfo.exception.TokenFailureException;
...
public abstract class BaseAssertion extends Assert {
	 ...
	 * @param message
	 */
	public static void validateToken(Object object, String message) {
		if (object == null) {
			throw new TokenFailureException(message);
		}
	}
	...
	public static void betweenLength(String value, int min, int max, String message) {
		if (min >= max) {
			throw new BizException("最小值: " + min + "不能大于等于最大值: " + max);
		}
	...
	public static void betweenOrEqualLength(String value, int min, int max, String message) {
		if (min > max) {
			throw new BizException("最小值: " + min + "不能大于最大值: " + max);
		}
		if (value.length() < min || value.length() > max) {
			throw new BizException(message);
		}
	...
	public static void in(String str, String[] enums, String message) {
		if (!Detect.notEmpty(enums)) {
			throw new BizException(message + "，枚举集合为空");
		}
		boolean match = false;
    ...
    public static synchronized String getGeneraterNextNumber() { 
        ...
        String orderCode = formatter.format(date) + String.format("%04d", BaseAssertion.getCount());
        return orderCode; 
    }
