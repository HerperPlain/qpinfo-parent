...
public abstract class Detect {
	...
	/** 导入中使用 */
	public static final String CONNECTORFLAG = "=" ;
	/** 导入中使用 */
	public static final String REFERENCELIST_SUFFIX = "_LIST" ;
	...
	public static String getUUID() {
		String codeId = String.valueOf(System.nanoTime()) ;
		int length = codeId.length() ;
		log.info("codeId length:" +codeId.length());
		int rightPadLength = 17 - length ;
		String rigthPadStr = codeId ;
		if (Detect.isPositive(rightPadLength)) {
			rigthPadStr = StringUtils.leftPad(codeId, 19, "0") ;
		}
		codeId = String.valueOf("8" + rigthPadStr);
		return codeId;
	}
	/**
	 * 随机生成20位数字ID
	...
	public static List<long[]> grouping(long[] values, int groupSize) {
		if (notEmpty(values)) {
			...
			BaseAssertion.isPositive(groupSize, "divideSize must be bigger than 0");
	...
	{
		BaseAssertion.isTrue(length > 0,"length must > 0 !");
		Random rand;
		rand= new Random(); 
